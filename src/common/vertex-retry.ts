...
const DEFAULT_MAX_ATTEMPTS = Math.max(
  1,
  parsePositiveInt(process.env.VERTEX_RETRY_MAX_ATTEMPTS, 4),
);
const DEFAULT_INITIAL_DELAY_MS = Math.max(
  50,
  parsePositiveInt(process.env.VERTEX_RETRY_INITIAL_DELAY_MS, 500),
);
const DEFAULT_MAX_DELAY_MS = Math.max(
  DEFAULT_INITIAL_DELAY_MS,
  parsePositiveInt(process.env.VERTEX_RETRY_MAX_DELAY_MS, 8000),
);
const DEFAULT_BACKOFF_MULTIPLIER = Number.isFinite(
  Number(process.env.VERTEX_RETRY_BACKOFF_MULTIPLIER),
)
...
const extractStatusCode = (error: any): number | undefined => {
  ...
  if (statusMatch?.[1]) return Number.parseInt(statusMatch[1], 10);
...
export const isRetryableVertexError = (error: unknown): boolean => {
  const statusCode = extractStatusCode(error);
  if (statusCode && RETRYABLE_HTTP_CODES.has(statusCode)) return true;
...
export const withVertexRetry = async <T>(
  operation: RetryableOperation<T>,
  options: VertexRetryOptions,
): Promise<T> => {
  ...
  const shouldRetry = options.shouldRetry ?? isRetryableVertexError;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      // Full jitter to desynchronize retries and avoid thundering herds.
  ...
  throw new Error(
    `Unexpected retry loop termination for operation "${options.operationName}"`,
  );
};
